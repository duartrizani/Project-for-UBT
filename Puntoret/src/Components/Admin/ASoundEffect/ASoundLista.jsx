...
const ASoundLista = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/sound/klista`);
  ...
  const handleDelete = () => {
    if (confirm("Are you sure you want to delete the selected employees?")) {
      axios.delete(`${import.meta.env.VITE_API_URL}/sound/delete_workers`, { data: { idsToDelete: selectedEmployeeIds } })
        .then(result => {
  ...
  const handleDeleteButton = (e) => {
    ...
    if (confirm("Are you sure you want to delete this employee?")) {
      axios.delete(`${import.meta.env.VITE_API_URL}/sound/delete_klista/${workerId}`)
        .then(result => {
  ...
  return (
    <div className="lg:px-5 mt-3 pl-2">
      ...
      <div className="flex justify-between">
        ...
        <Link to="/dashboard/soundeffect/add_klista" className="btn btn-success ">
          Shto Listë
        </Link>
          ...
          <tbody className="text-center">
            {sortedFilteredWorker.map((e) => (
              <tr key={e.id}>
                ...
                <td>
                  <Link
                    to={`/dashboard/soundeffect/edit_klista/${e.id}`}
                ...
                {activeDropdown === e.id && (
                  <>
                    ...
                    <tr className="flex items-center text-wrap justify-between">
                      ...
                      <td className="flex justify-end">
                        <Link to={`/dashboard/soundeffect/edit_klista/` + e.id} className="btn btn-info btn-sm me-2">
                          Edit
                        </Link>
                        <button

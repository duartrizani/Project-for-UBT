...
const AProgEmployee = () => {
  ...
  const handleDelete = (worker_id) => {
    if (confirm("Are you sure you want to delete this employee?")) {
      ...
      axios.delete(`${import.meta.env.VITE_API_URL}/prog/delete_worker/` + worker_id)
      axios.delete(`${import.meta.env.VITE_API_URL}/prog/delete_employee/` + worker_id)
        .then(result => {
          if (result.data.Status) {
  ...
  return (
    <div className="lg:px-5 mt-3 pl-2">
      ...
      <Link to="/dashboard/programer/add_employee" className="btn btn-success ">
        Shto Puntorë
      </Link>
          ...
          <tbody className="text-center">
            {employee.map((e, index) => (
              <tr>
                ...
                <td>
                  <Link
                    to={`/dashboard/programer/oret/` + e.worker_id}
                    className="btn btn-primary btn-sm me-2"
                  >
                    Orët
                  </Link>
                  <Link
                    to={`/dashboard/programer/edit_employee/` + e.worker_id}
            ...
            <>
              ...
              <tr className="flex items-center text-wrap justify-between">
                ...
                <td className="flex justify-end">
                  <Link to={`/dashboard/programer/oret/` + e.worker_id} className="btn btn-primary btn-sm me-2">
                    Orët
                  </Link>
                  <Link to={`/dashboard/programer/edit_employee/` + e.worker_id} className="btn btn-info btn-sm me-2">
                    Edit
                  </Link>
